...
from constants import PERIOCULAR_SHAPE, datasets_botheyes
from load_data import load_peri_dataset_both_eyes, load_dataset_both_eyes
...
def generate_normalized_data(no_resize: bool):
    """Used to generate the normalized _fix dataset. As opposed to the
    previous normalized data, this one does not go through unnecesary
    scaling and dtype changing. Additionally, we used bilinear interpol-
    ation instead of bicubic.

    no_resize True is used for VGG full tests. False is used for VGG+
    LSVM.
    """
    for dataset in datasets_botheyes:
        ...
        dataset_shape = find_dataset_shape('left_' + dataset)
        if no_resize:
            out_size = (max(dataset_shape[0], 32), dataset_shape[1], 3)
        else:
            out_size = (224, 224, 3)    
        ...
        for eye in all_data:
            ...
            if data_x.max() == 1:
                data_x = data_x * 255
            data_x = gray2rgb(data_x.reshape((-1, *dataset_shape)))
            out_data_x = np.zeros((data_x.shape[0], *out_size))
            for i in range(data_x.shape[0]):
                cur_img = data_x[i]
                if dataset_shape != out_size[:2]:
                    cur_img = resize(
                        cur_img, output_shape=out_size[:2], mode='edge',
                        order=1
                    )
                out_data_x[i, :, :, :] = cur_img
        ...
        out_name = f'{dataset}_pre_vgg' if no_resize else f'{dataset}_vgg' 
        np.savez_compressed(
            f'data/{out_name}.npz',
            all_data=all_data,
            males_set=males_set,
