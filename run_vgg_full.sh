...
run_vgg_full(){
    ...
    for nPart in $(seq 1 1); do  # CHANGE NUMBERS APPROPRIATELY
        # for d in $(seq 0 3); do
            ...
            # # # python ./vgg_full.py -pf step_by_step_vgg_1_lesslr/params.json -d $d -p $nPart -sbs
            python ./vgg_full.py -pf step_by_step_vgg_1_lesslr/params.json -d 1 -p "$nPart" -sbs
            python ./vgg_full.py -pf step_by_step_vgg_1_lesslr/params.json -d 3 -p "$nPart" -sbs
        # done
