...
def vgg_feat_lsvm_parall(data, partition: int, n_iters: Union[int, None],
                         ...
                         bad_bins_to_remove=0, use_ndiris=False,
                         ndiris_size=""):
    ...
    # Prepare data
    if both_eyes_mode:
        if use_ndiris and use_mask_pairs:
            from load_ndiris_data import load_ndiris_dataset
            train_x, train_y, _, _, test_x, test_y, _, _ = load_ndiris_dataset(
                ndiris_size, partition, use_mask_pairs, TEST_SIZE,
                bad_bins_to_remove=bad_bins_to_remove
            )

        elif use_ndiris:
            from load_ndiris_data import train_test_split, \
                permute_images, split_from_partition_df
            x_array, y_array, l_array, df = data
            train_df, test_df = train_test_split(df, TEST_SIZE, partition)
            train_x, train_y, _ = split_from_partition_df(
                x_array=x_array, y_array=y_array, m_array=None,
                l_array=l_array, part_df=train_df)
            test_x, test_y, _ = split_from_partition_df(
                x_array=x_array, y_array=y_array, m_array=None,
                l_array=l_array, part_df=test_df)
            train_x, train_y, test_x, test_y = permute_images(
                train_x=train_x, train_y=train_y, train_m=None, train_l=None,
                test_x=test_x, test_y=test_y, test_m=None, test_l=None
            )
...
def _perform_vgg_feat_lsvm_test(data_type, data_params, dataset_name: str,
                                ...
                                pairs_threshold=0.1, bad_bins_to_remove=False,
                                use_ndiris=False):
    ...
    args = [(data, i, n_iters, both_eyes_mode, params_set, subjobs,
             use_mask_pairs, pairs_threshold, bad_bins_to_remove, use_ndiris,
             dataset_name)
...
def main_vgg_feat_lsvm_test():
    ...
    if use_ndiris and not use_botheyes:
        use_botheyes = True
        print('[WARN] Set use_botheyes to true')
